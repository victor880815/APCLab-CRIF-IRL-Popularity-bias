...
#     ui_train_list = []
#     for user_str, items in PG_train_data_list.items(): #user對應到的item
#         user_id = int(user_str)
#         # print(user_id)
#         # print(items)
#         for item_id in items:
...
#     user_att_count_not_subset.set_config(config) #載入dataset
...
# rec.load_model(model_name, False)
# rec.init_eval(config.adj_index)
# usersim = UserSim(UserSimConfig(config)) #用戶模擬器，可以同時進行推薦和推斷用戶偏好
# dm = DialogueManager(DialogueManagerConfig(config), rec, agent, usersim, ch)
# set_random_seed(1)
# PG_train(agent, args.mode, dm)
