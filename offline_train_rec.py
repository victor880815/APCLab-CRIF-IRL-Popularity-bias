...
def train_one_epoch(epoch_num):
    ...
    for batch_data in tqdm(train_loader, ncols=0):
        ...
        # print('neg_item_list1neg_item_list1', neg_item_list1)
        # print('neg_item_list2neg_item_list2',neg_item_list2)
...
def main():
    ...
    parser.add_argument('--data_name', type=str, default=LAST_FM, choices=[LAST_FM, LAST_FM_STAR, YELP, YELP_STAR],  #選擇資料集
                        help='One of {lastfm, lastfm_star, yelp, yelp_star}.')
    args = parser.parse_args()
    print(args.data_name)
    if args.data_name == LAST_FM:
        config = ConfigLastFM()  #載入dataset config
        user_att_count.set_config(config) #載入dataset
