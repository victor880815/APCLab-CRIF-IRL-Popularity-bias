...
class DialogueManager:
    ...
    def step(self, action_index, pop_item_list, unpop_item_list):
        pcu = 0
        is_ask = action_index == self.ask_action_index
        # print('is_askis_askis_ask', is_ask)
        ...
        if is_ask:
            ...
            ask_reward = self.agent.get_reward(self.get_reward_state(self.ask_action_index)) - torch.tensor(0.1).cuda() #透過RWN 獲得ask reward
            feedback_rec_reward = self.agent.get_reward(self.get_reward_state(self.rec_with_att_feedback_action_index)) - torch.tensor(0.1).cuda() #透過RWN 獲得feedback_rec reward
            reward = ask_reward
        ...
        if self.target_item in unpop_item_list:
            pcu = -1
        if self.target_item in pop_item_list:
            pcu = 1

        # print(pcu)
        return IsOver, self.get_state(), reward, success, be_len, be_rank, ask_len, ask_rank, \
               feedback_rec_len, feedback_rec_rank, ask_reward, feedback_rec_reward, step_state, pcu
    ...
    def get_reward_state(self, action_index):
        state_user = self.convhis.get_user_vertor()
        # print('state_userstate_user', state_user)
        state_convhis = self.convhis.get_convhis_vector().copy()
        # print('state_convhisstate_convhis', state_convhis)
        state_len = self.convhis.get_length_vector().copy()
        # print('state_lenstate_len', state_user)
        dialogue_state = state_user + state_convhis + state_len + [action_index]
        # print('action_indexaction_index', action_index)
        dialogue_state = torch.tensor(dialogue_state)
        # print('dialogue_statedialogue_state', dialogue_state)
        return dialogue_state
